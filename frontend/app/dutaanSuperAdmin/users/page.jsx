...
export default function admin() {
    ...
    let getUsers = async() => {
        ...
        let response = await api.get("/getAllUsers/")
        ...
        <tbody>
          {users.map((e, index)=>(
            <tr key={e.id} className="border-2 border-black">
              ...
              <td className='p-5 date border-2 border-black'>{e.date_joined}</td>
              <td className='p-5 date border-2 border-black'>{e.last_login}</td>
              <td className='p-5 border-2 border-black'>Customer</td>
              <td className='p-5 border-2 border-black'><Button><Link href={`users/edit?id=${e.id}`}>Edit</Link></Button></td>
            </tr>
          ))}
